# coding: utf-8

class FoursquareNotification:
    """
     Contains notifications of different types including insights, notificationTray, mayorship, score, message, leaderboard, etc.

    """

    def __init__(self, base):
        if base:
            self.base = base
        else:
            self.base = {}

    def getNotificationItem(self):
        """
        Get an individual notification item
        """
        return FoursquareNotificationItem(self.base.get("item", []))

    def getType(self):
        """
        The type of notification
        """
        return self.base.get("type", [])

    def getBase(self):
        """
        Internal utility method; retrieve the base JSON object for this element of the response data.
        """
        return self.base

    def getJSONObject(self, json, key):
        """
        Internal utility method; retrieve a sub-object from a JSON object/array; returns an empty object if key is not present
        """
        toReturn = {}

        if json is not None:
            toReturn = json.get(key, {})

        return toReturn

from temboo.outputs.Foursquare.FoursquareNotificationItem import FoursquareNotificationItem