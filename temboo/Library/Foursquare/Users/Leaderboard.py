...
from temboo.outputs.Foursquare.FoursquareLeaderboardItem import FoursquareLeaderboardItem
...
class LeaderboardResultSet(ResultSet):
    ...
    def getLeaderboardItems(self):
        """
        Get leaderboard items
        """
        return [FoursquareLeaderboardItem(le) for le in self.getJSONObject(self.getJSONObject(self.getJSONFromString(self._output.get('Response', [])), "response"), "leaderboard").get("items", [])]
