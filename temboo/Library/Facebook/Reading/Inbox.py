...
from temboo.outputs.Facebook.FacebookInboxItem import FacebookInboxItem
from temboo.outputs.Facebook.FacebookSummary import FacebookSummary
...
class InboxResultSet(ResultSet):
    ...
    def getInboxItems(self):
        """
        Get an inbox item
        """
        return [FacebookInboxItem(le) for le in self.getJSONFromString(self._output.get('Response', [])).get("data", [])]

    def getSummary(self):
        """
        Get the summary for this inbox
        """
        return FacebookSummary(self.getJSONFromString(self._output.get('Response', [])).get("summary", []))
